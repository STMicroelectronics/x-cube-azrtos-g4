...
by calling the TX_LOW_POWER_USER_ENTER that makes the system enter the stop mode via the HAL_PWREx_EnterSTOP1Mode() API.

As soon as the User button is pressed, the 'Semaphore' is released, and the 'MainThread' passes in Active state.

The scheduler exits the stop1 mode by calling the TX_LOW_POWER_EXIT.

In System STOP1 mode, all I/O pins keep the same state as in Run mode.

When exiting the STOP1 mode, the MCU is in Run mode (Range 1 or Range 2 depending on VOS bit in PWR_CR1).
...
4. The Debug Module must be stopped during STOP1 mode, otherwise, It will generate interrupts that cause the Wake-up of the system immediately after entering the STOP1 mode.

5. The user push-button interrupt priority must be set to the highest possible value, otherwise, after Exit STOP1 mode the processor will not execute the interrupt handler.
