...
static VOID Led_Toggle(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin, UINT iter, ULONG delay);
...
void ThreadTwo_Entry(ULONG thread_input)
{
  ...
  while(1)
  {
    ...
    if (APP_SYNC_GET(&SyncObject, TX_NO_WAIT) == TX_SUCCESS)
    {
      ...
      /*Sync object acquired toggle the LED_GREEN each 200ms for 5s*/
      Led_Toggle(LED1_GPIO_Port, LED1_Pin, 25,20);
      /*release the sync object*/
      APP_SYNC_PUT(&SyncObject);
...
/**
  ...
  * @param  delay: Delay for each toggle iteration
  * @retval None
  */
static VOID Led_Toggle(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin, UINT iter, ULONG delay)
{
  ...
  for (i =0; i < iter; i++)
  {
    ...
    /* Toggle the Led each (delay*10) ms */
    App_Delay(delay);
  }
